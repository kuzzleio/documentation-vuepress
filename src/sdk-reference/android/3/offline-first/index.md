...
_Offline mode_ refers to the time between a `disconnected` and a `reconnected` event (see [Events](/sdk-reference/android/3/events)).

---

## Subscriptions

A subscription opens a permanent pipe between the client and Kuzzle. Whenever a real-time message or a modified document matches a subscription filter, a notification is sent by Kuzzle to the client (for instance, see the [Collection.subscribe](/sdk-reference/android/3/collection/subscribe) method).
...
- Queue all requests automatically when going offline by setting the `autoQueue` option to `true` (see [Kuzzle SDK constructor](/sdk-reference/android/3/kuzzle))
- Start and stop queuing manually, by using the [startQueuing](/sdk-reference/android/3/kuzzle/start-queuing) and [stopQueuing](/sdk-reference/android/3/kuzzle/stop-queuing) methods
...
However, you can choose to omit certain request by using the [`queueFilter`](/sdk-reference/android/3/kuzzle#properties) property. This property can be set to a function that accepts the request as an input value and returns a boolean result which indicates whether or not the request should be queud.
...
You can be notified about what's going on in the offline queue, by using the [`offlineQueuePush`](/sdk-reference/android/3/events) and the [`offlineQueuePop`](/sdk-reference/android/3/events) events.
...
You can set the `offlineMode` option to `auto` when instantiating the [Kuzzle SDK instance](/sdk-reference/android/3/kuzzle). This sets the offline mode configuration to the following presets:
