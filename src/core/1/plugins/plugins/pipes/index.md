...
<div class="alert alert-warning">If a pipe takes too long to respond, Kuzzle will eventually abort the entire task with a <a href="/plugins/1/errors/gatewaytimeouterror">GatewayTimeout</a> error. The timeout value can be changed in the <a href="/core/1/guide/essentials/configuration">configuration files.</a></div>
...
- by returning a promise, resolved (or rejected) with a valid [Request](/core/1/guide/essentials/request-and-response-format) upon the completion of the pipe

<div class="alert alert-warning">You must either call the callback with a valid <a href="/core/1/guide/essentials/request-and-response-format">Request</a> or return a promise resolving to one.</div>
