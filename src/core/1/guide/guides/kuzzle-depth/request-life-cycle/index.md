...
- **Synchronous**: Clients send requests to the Kuzzle server, which processes the data using the [Document Controller](/api/1/controller-document) and then sends a response back to the client.

- **Asynchronous**: Clients subscribe to the Kuzzle server via the [Real-time/Subscribe](/api/1/controller-realtime/publish) API action and receive data asynchronously as a result of a [Real-time/Publish](/api/1/controller-realtime/publish) API action or a [Document Controller](/api/1/controller-document) event.
...
- The client application opens a socket (or MQ) connection, sends a subscription request (see the [API Documentation](/api/1/controller-realtime/subscribe)), and then listens for the `< requestId >` event on the socket. The subscription request is a message that contains a filter description that defines which events should trigger a response. For instance, the following filter will trigger a response anytime content is posted to the `users` collection that contains the field `hobby` with value `computer` (see the [Koncorde Reference](/koncorde/1/) for more details):
...
The following diagram shows how the Kuzzle server triggers a response as a result of a publish request made using the [Real-time/Publish](/api/1/controller-realtime/publish) action.
