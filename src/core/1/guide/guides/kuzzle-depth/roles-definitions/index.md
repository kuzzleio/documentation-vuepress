...
In the [Getting Started Guide](/core/1/guide/essentials/security/#user-permissions), we discussed how to assign basic permissions to users through roles and profiles. We are now going to look at more complex and dynamic permissions.
...
- `test` is the body of [the permission function](/core/1/guide/kuzzle-depth/roles-definitions/#the-permission-function)
- `args` is the parameter given to [the fetch definition function](/core/1/guide/kuzzle-depth/roles-definitions/#the-fetch-definition)
...
- `$currentUserId`: The current user [`<kuid>`](/core/1/guide/essentials/user-authentication/#kuzzle-user-identifier-kuid) (equivalent to `request.context.token.userId`).
...
Documents fetched from the storage layer are stored in the `args` object as defined by the [fetch definition](/core/1/guide/kuzzle-depth/roles-definitions/#the-fetch-definition).
...
The `search` action type performs a search on the persistence layer and returns the resulting documents. It is a typical [document search](/core/1/guide/essentials/persisted/#document-search). For example:
...
Please refer to our [Elasticsearch Cookbook](/core/1/guide/elasticsearch/) for more help on how to build your search query.
