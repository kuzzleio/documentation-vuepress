...
In the [Getting Started Guide](/guide/1/essentials/security/#user-permissions), we discussed how to assign basic permissions to users through roles and profiles. We are now going to look at more complex and dynamic permissions.
...
- `test` is the body of [the permission function](/guide/1/kuzzle-depth/roles-definitions/#the-permission-function)
- `args` is the parameter given to [the fetch definition function](/guide/1/kuzzle-depth/roles-definitions/#the-fetch-definition)
...
- `$currentUserId`: The current user [`<kuid>`](/guide/1/essentials/user-authentication/#kuzzle-user-identifier-kuid) (equivalent to `request.context.token.userId`).
...
Documents fetched from the storage layer are stored in the `args` object as defined by the [fetch definition](/guide/1/kuzzle-depth/roles-definitions/#the-fetch-definition).
...
The `search` action type performs a search on the persistence layer and returns the resulting documents. It is a typical [document search](/guide/1/essentials/persisted/#document-search). For example:
...
Please refer to our [Elasticsearch Cookbook](/guide/1/elasticsearch/) for more help on how to build your search query.
