...
To secure your Kuzzle installation you will need to create an administrator account by either using the [Kuzzle Admin Console](/guide/1/essentials/installing-console/#create-an-admin-account) or using the [CLI](/guide/1/essentials/cli/#createfirstadmin) tool.
...
All `roles` and `profiles` can be edited in the [Kuzzle Admin Console](/guide/1/essentials/installing-console).
...
- an object describing a dynamic right definition. For more information check out the [advanced roles documentation](/guide/1/kuzzle-depth/roles-definitions)).
...
- <DeprecatedBadge version="1.4.0" /> Using [Permission Closures](guide/1/kuzzle-depth/roles-definitions), you can add functions directly into role definitions
- If all you need is to make sure that submitted documents follow a strict set of formatting rules, you can add [document validators](/guide/1/datavalidation/)
